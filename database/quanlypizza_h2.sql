-- H2 Database - Quản lý Pizza
-- Converted from MySQL for H2 compatibility (MODE=MySQL)

-- Bảng loai
CREATE TABLE IF NOT EXISTS loai (
  MaLoai INT AUTO_INCREMENT PRIMARY KEY,
  TenLoai VARCHAR(255) NOT NULL
);

-- Bảng sanpham
CREATE TABLE IF NOT EXISTS sanpham (
  MaSP INT AUTO_INCREMENT PRIMARY KEY,
  TenSP VARCHAR(255) NOT NULL,
  MaLoai INT NOT NULL,
  SoLuong INT NOT NULL,
  DonViTinh VARCHAR(255) NOT NULL,
  HinhAnh VARCHAR(255) NOT NULL,
  DonGia INT NOT NULL,
  CONSTRAINT sanpham_ibfk_1 FOREIGN KEY (MaLoai) REFERENCES loai(MaLoai) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Bảng khachhang
CREATE TABLE IF NOT EXISTS khachhang (
  MaKH INT AUTO_INCREMENT PRIMARY KEY,
  Ho VARCHAR(255) NOT NULL,
  Ten VARCHAR(255) NOT NULL,
  GioiTinh VARCHAR(3) NOT NULL,
  TongChiTieu INT NOT NULL DEFAULT 0,
  TinhTrang INT NOT NULL
);

-- Bảng nhanvien
CREATE TABLE IF NOT EXISTS nhanvien (
  MaNV INT AUTO_INCREMENT PRIMARY KEY,
  Ho VARCHAR(255) NOT NULL,
  Ten VARCHAR(255) NOT NULL,
  GioiTinh VARCHAR(3) NOT NULL,
  ChucVu VARCHAR(255) NOT NULL
);

-- Bảng phanquyen
CREATE TABLE IF NOT EXISTS phanquyen (
  Quyen VARCHAR(255) PRIMARY KEY,
  NhapHang INT NOT NULL,
  QLSanPham INT NOT NULL,
  QLNhanVien INT NOT NULL,
  QLKhachHang INT NOT NULL,
  ThongKe INT NOT NULL
);

-- Bảng taikhoan
CREATE TABLE IF NOT EXISTS taikhoan (
  MaNV INT PRIMARY KEY,
  TenDangNhap VARCHAR(255) NOT NULL,
  MatKhau VARCHAR(255) NOT NULL,
  Quyen VARCHAR(255) NOT NULL,
  TrangThai INT NOT NULL,
  CONSTRAINT taikhoan_ibfk_1 FOREIGN KEY (MaNV) REFERENCES nhanvien(MaNV) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT taikhoan_ibfk_2 FOREIGN KEY (Quyen) REFERENCES phanquyen(Quyen) ON DELETE CASCADE
);

-- Bảng hoadon
CREATE TABLE IF NOT EXISTS hoadon (
  MaHD INT AUTO_INCREMENT PRIMARY KEY,
  MaKH INT NOT NULL,
  MaNV INT NOT NULL,
  NgayLap DATE NOT NULL,
  TongTien INT NOT NULL,
  GhiChu VARCHAR(1000) NOT NULL,
  CONSTRAINT hoadon_ibfk_1 FOREIGN KEY (MaNV) REFERENCES nhanvien(MaNV) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT hoadon_ibfk_2 FOREIGN KEY (MaKH) REFERENCES khachhang(MaKH) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Bảng cthoadon
CREATE TABLE IF NOT EXISTS cthoadon (
  MaHD INT NOT NULL,
  MaSP INT NOT NULL,
  SoLuong INT NOT NULL,
  DonGia INT NOT NULL,
  ThanhTien INT NOT NULL,
  PRIMARY KEY (MaHD, MaSP),
  CONSTRAINT cthoadon_ibfk_1 FOREIGN KEY (MaHD) REFERENCES hoadon(MaHD) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Bảng nhacungcap
CREATE TABLE IF NOT EXISTS nhacungcap (
  MaNCC INT AUTO_INCREMENT PRIMARY KEY,
  TenNCC VARCHAR(255) NOT NULL,
  DiaChi VARCHAR(255) NOT NULL,
  DienThoai VARCHAR(255) NOT NULL
);

-- Bảng phieunhap
CREATE TABLE IF NOT EXISTS phieunhap (
  MaPN INT AUTO_INCREMENT PRIMARY KEY,
  MaNCC INT NOT NULL,
  MaNV INT NOT NULL,
  NgayLap DATE NOT NULL,
  TongTien INT NOT NULL,
  CONSTRAINT phieunhap_ibfk_1 FOREIGN KEY (MaNCC) REFERENCES nhacungcap(MaNCC) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT phieunhap_ibfk_2 FOREIGN KEY (MaNV) REFERENCES nhanvien(MaNV) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Bảng ctphieunhap
CREATE TABLE IF NOT EXISTS ctphieunhap (
  MaPN INT NOT NULL,
  MaSP INT NOT NULL,
  SoLuong INT NOT NULL,
  DonGia INT NOT NULL,
  ThanhTien INT NOT NULL,
  PRIMARY KEY (MaPN, MaSP),
  CONSTRAINT ctphieunhap_ibfk_2 FOREIGN KEY (MaPN) REFERENCES phieunhap(MaPN) ON DELETE CASCADE ON UPDATE CASCADE
);

-- Bảng giamgia
CREATE TABLE IF NOT EXISTS giamgia (
  MaGiam INT AUTO_INCREMENT PRIMARY KEY,
  TenGiamGia VARCHAR(500) NOT NULL,
  PhanTramGiam INT NOT NULL,
  DieuKien INT NOT NULL,
  NgayBD DATE NOT NULL,
  NgayKT DATE NOT NULL
);

-- =============================================
-- DỮ LIỆU MẪU
-- =============================================

-- Loai
INSERT INTO loai (MaLoai, TenLoai) VALUES
(1, 'Pizza'),
(2, N'Nước uống'),
(3, N'Nguyên liệu');

-- Nhan vien
INSERT INTO nhanvien (MaNV, Ho, Ten, GioiTinh, ChucVu) VALUES
(0, 'Admin', '', '', 'Admin'),
(1, N'Sau Đại', N'Phát', 'Nam', N'Quản Lý'),
(2, N'Trần Quang', 'Vinh', 'Nam', N'Quản Lý'),
(3, N'Trần Hữu', N'Khương', 'Nam', N'Quản Lý'),
(4, N'Nhữ Quốc Anh', N'Tài', 'Nam', N'Nhân Viên'),
(12, N'Trịnh Thu', N'Huyền', N'Nữ', N'Nhân Viên'),
(13, N'Nguyễn Ngọc Mai', 'Linh', N'Nữ', N'Nhân Viên'),
(14, N'Hà Văn', N'Hùng', 'Nam', N'Nhân Viên'),
(15, N'Lê Thị Thuý', N'Diễm', N'Nữ', N'Quản Lý');

-- Phan quyen
INSERT INTO phanquyen (Quyen, NhapHang, QLSanPham, QLNhanVien, QLKhachHang, ThongKe) VALUES
('Default', 0, 0, 0, 0, 0),
(N'Nhân viên', 0, 0, 0, 1, 0),
(N'Quản lý', 1, 0, 1, 1, 1),
(N'Quản trị', 1, 1, 1, 1, 1);

-- Tai khoan
INSERT INTO taikhoan (MaNV, TenDangNhap, MatKhau, Quyen, TrangThai) VALUES
(0, 'admin', 'admin', N'Quản trị', 1),
(1, 'ql01', 'ql01', N'Quản lý', 1),
(2, 'ql02', 'ql02', N'Quản lý', 1),
(3, 'ql03', 'ql03', N'Quản lý', 1),
(4, 'nv01', 'nv01', N'Nhân viên', 1);

-- Khach hang
INSERT INTO khachhang (MaKH, Ho, Ten, GioiTinh, TongChiTieu, TinhTrang) VALUES
(1, N'Nguyễn Văn', 'An', 'Nam', 1486400, 1),
(2, N'Trần Ngọc', N'Bình', N'Nữ', 1497300, 1),
(3, N'Lê Thị', N'Giải', N'Nữ', 1468800, 1),
(4, N'Hà Thanh', N'Thoát', 'Nam', 1013600, 1),
(7, N'Trịnh Thị', N'Yên', N'Nữ', 3219700, 1),
(8, N'Trương Đình', 'Vui', 'Nam', 1877000, 1),
(9, N'Lại Thị Ngọc', N'Miền', N'Nữ', 5743500, 1),
(10, N'Trần Quang', N'Cực', 'Nam', 3136800, 1),
(11, N'Nguyễn Ngọc', N'Lạc', 'Nam', 2260300, 1);

-- San pham
INSERT INTO sanpham (MaSP, TenSP, MaLoai, SoLuong, DonViTinh, HinhAnh, DonGia) VALUES
(111, N'Pizza Hải Sản Pesto Xanh', 1, 31, N'Cái', 'pizza0.png', 169000),
(112, N'Pizza Hải Sản Nhiệt Đới', 1, 39, N'Cái', 'pizza1.png', 129000),
(113, N'Pizza Hải Sản Cocktail', 1, 34, N'Cái', 'pizza2.png', 119000),
(114, N'Pizza Tôm Cocktail', 1, 36, N'Cái', 'pizza3.png', 159000),
(115, 'Pizza Aloha', 1, 38, N'Cái', 'pizza4.png', 119000),
(116, N'Pizza Thịt Xông Khói', 1, 39, N'Cái', 'pizza5.png', 130000),
(117, N'Pizza Thịt Nguội', 1, 34, N'Cái', 'pizza6.png', 149000),
(118, N'Pizza Gà Nướng 3 Vị', 1, 35, N'Cái', 'pizza7.png', 129000),
(119, N'Pizza hải sản Pesto gấp đôi nhân', 1, 13, N'Cái', 'pizza8.jpg', 239000),
(120, N'Pizza gấp đôi nhân phủ cơn lốc hải sản', 1, 37, N'Cái', 'pizza9.jpg', 239000),
(121, N'Pizza gấp đôi nhân phủ hải sản xốt tiêu đen', 1, 41, N'Cái', 'pizza10.jpg', 229000),
(122, N'Pizza bò nướng tiêu đen', 1, 43, N'Cái', 'pizza11.jpg', 249000),
(123, N'Pizza cá ngừ thanh cua', 1, 33, N'Cái', 'pizza12.jpg', 269000),
(124, 'Pepsi lon 330ml', 2, 150, 'Lon', 'douong0.jpeg', 19000),
(125, 'Pepsi chai 1.5l', 2, 250, 'Chai', 'douong1.jpeg', 29000),
(126, '7Up lon 330ml', 2, 150, 'Lon', 'douong2.jpeg', 19000),
(127, '7Up chai 1.5l', 2, 110, 'Chai', 'douong3.jpeg', 29000),
(128, 'Pepsi no calories 330ml', 2, 180, 'Lon', 'douong4.jpg', 19000),
(129, 'Mirinda cam 330ml', 2, 100, 'Lon', 'douong5.jpg', 19000),
(130, 'Mirinda kem soda 330ml', 2, 80, 'Lon', 'douong6.jpg', 19000),
(131, N'Nước suối Aquafina', 2, 52, 'Chai', 'douong7.jpg', 10000),
(132, N'Trà sữa trà đen', 2, 6, 'Ly', 'douong8.jpg', 25000),
(135, N'Bột mì Meizan 500g', 3, 100, N'Gói', 'default.png', 10000),
(136, N'Men nở Mauripan 50g*5', 3, 200, N'Hộp', 'default.png', 23000),
(137, N'Dầu olive Olivoilà 250ml', 3, 80, 'Chai', 'default.png', 84500);

-- Nha cung cap
INSERT INTO nhacungcap (MaNCC, TenNCC, DiaChi, DienThoai) VALUES
(1, N'Bột mì Khánh Toàn', N'273 An Dương Vương, phường 3, quận 5, TP.HCM', '0396527908'),
(2, N'Công ty giải khát Tường Vy', N'12/49/2 Võ Văn Kiệt, P. Cô Giang, Q1', '0902669733'),
(5, N'Nguyên liệu bánh Việt Hoàng', N'492 Xô Viết Nghệ Tĩnh, P.25, Q. Bình Thạnh', '0397682193');

-- Giam gia
INSERT INTO giamgia (MaGiam, TenGiamGia, PhanTramGiam, DieuKien, NgayBD, NgayKT) VALUES
(1, N'Không giảm giá', 0, 0, '2020-01-01', '2042-12-31'),
(2, N'Giảm 20% đơn hàng từ 150.000đ', 20, 150000, '2021-04-22', '2021-05-13'),
(4, N'Giảm 10% đơn hàng từ 80.000đ', 10, 80000, '2021-03-26', '2021-05-31'),
(6, N'30/04-01/05 giảm 30% đơn hàng từ 300.000đ', 30, 300000, '2021-04-20', '2021-05-08');

-- Hoa don
INSERT INTO hoadon (MaHD, MaKH, MaNV, NgayLap, TongTien, GhiChu) VALUES
(18, 4, 3, '2021-01-18', 1013600, N'Đã thanh toán'),
(19, 1, 1, '2021-01-20', 482400, N'Đã thanh toán'),
(20, 2, 1, '2021-01-28', 840000, N'Đã thanh toán'),
(21, 3, 2, '2021-02-07', 1045600, N'Đã thanh toán'),
(22, 1, 1, '2021-02-14', 1004000, N'Đã thanh toán'),
(23, 7, 3, '2021-02-16', 576800, N'Đã thanh toán'),
(24, 7, 2, '2021-03-08', 601300, N'Đã thanh toán'),
(25, 2, 2, '2021-03-09', 391300, N'Đã thanh toán'),
(26, 8, 4, '2021-03-12', 1286600, N'Đã thanh toán'),
(27, 8, 3, '2021-03-24', 590400, N'Đã thanh toán'),
(28, 7, 4, '2021-02-04', 2041600, N'Đã thanh toán'),
(29, 9, 2, '2021-04-10', 633500, N'Đã thanh toán'),
(30, 10, 4, '2021-04-15', 3136800, N'Đã thanh toán'),
(31, 11, 4, '2021-05-02', 782600, N'Đã thanh toán'),
(32, 11, 0, '2021-05-03', 1477700, N'Đã thanh toán'),
(33, 9, 0, '2021-05-06', 5110000, N'Đã thanh toán'),
(35, 3, 0, '2021-05-08', 304200, N'Đã thanh toán'),
(37, 2, 0, '2021-05-08', 117000, N'Đã thanh toán');

-- Chi tiet hoa don
INSERT INTO cthoadon (MaHD, MaSP, SoLuong, DonGia, ThanhTien) VALUES
(18, 114, 2, 159000, 318000),
(18, 122, 3, 249000, 747000),
(18, 124, 6, 19000, 114000),
(18, 130, 2, 19000, 38000),
(18, 132, 2, 25000, 50000),
(19, 115, 2, 119000, 238000),
(19, 120, 1, 239000, 239000),
(19, 128, 4, 19000, 76000),
(19, 131, 5, 10000, 50000),
(20, 118, 1, 129000, 129000),
(20, 120, 2, 239000, 478000),
(20, 129, 5, 19000, 38000),
(20, 130, 5, 19000, 95000),
(20, 131, 10, 10000, 100000),
(21, 112, 3, 129000, 387000),
(21, 117, 2, 149000, 298000),
(21, 126, 10, 19000, 190000),
(21, 127, 3, 29000, 87000),
(21, 128, 15, 19000, 285000),
(21, 131, 6, 10000, 60000),
(22, 114, 5, 159000, 795000),
(22, 127, 10, 29000, 290000),
(22, 129, 6, 19000, 95000),
(22, 132, 3, 25000, 75000),
(23, 112, 3, 129000, 387000),
(23, 113, 1, 119000, 119000),
(23, 118, 1, 129000, 129000),
(23, 128, 6, 19000, 114000),
(23, 131, 5, 10000, 50000),
(23, 132, 2, 25000, 25000),
(24, 115, 1, 119000, 119000),
(24, 116, 1, 130000, 130000),
(24, 119, 1, 239000, 239000),
(24, 126, 6, 19000, 114000),
(24, 127, 3, 29000, 87000),
(24, 131, 7, 10000, 70000),
(24, 132, 4, 25000, 100000),
(25, 118, 1, 129000, 129000),
(25, 121, 1, 229000, 229000),
(25, 130, 4, 19000, 76000),
(25, 131, 5, 10000, 50000),
(25, 132, 3, 25000, 75000),
(26, 111, 3, 169000, 507000),
(26, 120, 3, 239000, 717000),
(26, 122, 2, 249000, 498000),
(26, 127, 4, 29000, 116000),
(27, 113, 1, 119000, 119000),
(27, 119, 1, 239000, 239000),
(27, 123, 1, 269000, 269000),
(27, 126, 1, 19000, 19000),
(27, 131, 1, 10000, 10000),
(28, 113, 10, 119000, 1190000),
(28, 116, 4, 130000, 520000),
(28, 119, 3, 239000, 717000),
(28, 132, 5, 25000, 125000),
(29, 114, 4, 159000, 636000),
(29, 115, 1, 119000, 119000),
(29, 116, 1, 130000, 130000),
(29, 131, 2, 10000, 20000),
(30, 116, 1, 130000, 130000),
(30, 117, 7, 149000, 1043000),
(30, 123, 10, 269000, 2690000),
(30, 125, 1, 29000, 29000),
(30, 127, 2, 29000, 29000),
(31, 111, 1, 169000, 169000),
(31, 115, 6, 119000, 595000),
(31, 117, 1, 149000, 149000),
(31, 118, 1, 129000, 129000),
(31, 130, 4, 19000, 76000),
(32, 120, 5, 239000, 1195000),
(32, 121, 4, 229000, 916000),
(33, 116, 1, 130000, 130000),
(33, 119, 30, 239000, 7170000),
(35, 113, 1, 119000, 119000),
(35, 117, 1, 149000, 149000),
(35, 131, 8, 10000, 70000),
(37, 116, 1, 130000, 130000);

-- Phieu nhap
INSERT INTO phieunhap (MaPN, MaNCC, MaNV, NgayLap, TongTien) VALUES
(4, 2, 0, '2021-04-19', 1027000),
(5, 2, 0, '2021-04-23', 268000),
(7, 2, 0, '2021-04-26', 90000),
(8, 2, 0, '2021-05-02', 1070000),
(9, 1, 0, '2021-05-04', 1000000),
(10, 5, 0, '2021-05-07', 11360000);

-- Chi tiet phieu nhap
INSERT INTO ctphieunhap (MaPN, MaSP, SoLuong, DonGia, ThanhTien) VALUES
(4, 128, 100, 10000, 1000000),
(4, 129, 3, 9000, 27000),
(5, 124, 6, 10000, 60000),
(5, 125, 3, 15000, 45000),
(5, 126, 7, 10000, 70000),
(5, 127, 2, 15000, 30000),
(5, 130, 3, 9000, 27000),
(5, 131, 9, 4000, 36000),
(7, 126, 9, 10000, 90000),
(8, 124, 30, 10000, 300000),
(8, 126, 50, 10000, 500000),
(8, 129, 30, 9000, 270000),
(9, 135, 100, 10000, 1000000),
(10, 136, 200, 23000, 4600000),
(10, 137, 80, 84500, 6760000);

-- Reset AUTO_INCREMENT sequences
ALTER TABLE loai ALTER COLUMN MaLoai RESTART WITH 11;
ALTER TABLE sanpham ALTER COLUMN MaSP RESTART WITH 145;
ALTER TABLE khachhang ALTER COLUMN MaKH RESTART WITH 14;
ALTER TABLE nhanvien ALTER COLUMN MaNV RESTART WITH 17;
ALTER TABLE hoadon ALTER COLUMN MaHD RESTART WITH 38;
ALTER TABLE nhacungcap ALTER COLUMN MaNCC RESTART WITH 6;
ALTER TABLE giamgia ALTER COLUMN MaGiam RESTART WITH 7;
ALTER TABLE phieunhap ALTER COLUMN MaPN RESTART WITH 11;
ALTER TABLE taikhoan ALTER COLUMN MaNV RESTART WITH 34;
